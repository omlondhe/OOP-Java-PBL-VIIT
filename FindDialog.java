...
// Find dialog class UI and Action handling
public class FindDialog extends JPanel implements ActionListener {
	...
	public FindDialog(JTextArea jta) {
		...
		textPanel.add(new JLabel(" "));// spacing label
		textPanel.add(new JLabel(" "));// spacing label

		setLayout(new BorderLayout());

		add(new JLabel("       "), BorderLayout.NORTH);// spacing label
	...
	// enabling ans disabling the button
	void enableDisableButtons() {
		findNextButton.setEnabled(findWhat.getText().length() != 0);
		replaceButton.setEnabled(findWhat.getText().length() != 0);
		replaceAllButton.setEnabled(findWhat.getText().length() != 0);
	}
	...
	// function for finding the next word
	int findNext() {
	...
	// function to find next word with the selection
	...
	// function to replace the next word
	void replaceNext() {
		...
		String temp = jta.getSelectedText(); // get the selected text
	...
	// function to replace all the next words
	int replaceAllNext() {
		if (up.isSelected())
